...
quixote.protocol.response.Response objects
"""
import os
import weakref
import webbrowser
...
from quixote.utils.python import to_bytes, to_native_str
from quixote.utils.decorators import deprecated
...
def body_or_str(*a, **kw):
    from quixote.utils.iterators import _body_or_str
...
def get_meta_refresh(response):
    ...
    if response not in _metaref_cache:
        text = response.text[0:4096]
        _metaref_cache[response] = html.get_meta_refresh(text, response.url, response.encoding,
