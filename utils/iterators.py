...
from quixote.protocol import TextResponse, Response
from quixote.selector import Selector
from quixote.utils.python import re_rsearch, to_unicode
...
def csviter(obj, delimiter=None, headers=None, encoding=None, quotechar=None):
    ...
    while True:
        ...
        if len(row) != len(headers):
            logger.warning("ignoring row %(csvlnum)d (length: %(csvrow)d, should be: %(csvheader)d)",
...
def _body_or_str(obj, unicode=True):
    ...
    assert isinstance(obj, expected_types), "obj must be %s, not %s" % (" or ".join(t.__name__ for t in expected_types),
                                                                        type(obj).__name__)
    if isinstance(obj, Response):
        if not unicode:
            return obj.body
